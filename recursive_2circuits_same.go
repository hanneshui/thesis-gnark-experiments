...
type OuterCircuit_twoInner_same[FR emulated.FieldParams, G1El algebra.G1ElementT, G2El algebra.G2ElementT, GtEl algebra.GtElementT] struct {
...
func (c *OuterCircuit_twoInner_same[FR, G1El, G2El, GtEl]) Define(api frontend.API) error {
	verifier, err := stdgroth16.NewVerifier[FR, G1El, G2El, GtEl](api)
	if err != nil {
		return fmt.Errorf("new verifier: %w", err)
	}
	verifier.AssertProof(c.VerifyingKeyA, c.ProofA, c.InnerWitnessA)
	verifier.AssertProof(c.VerifyingKeyB, c.ProofB, c.InnerWitnessB)
	return nil

}

func Compute_Outer_2Inner_same(innerCcsA constraint.ConstraintSystem, innerVKA groth16.VerifyingKey, innerWitnessA witness.Witness, innerProofA groth16.Proof, innerCcsB constraint.ConstraintSystem, innerVKB groth16.VerifyingKey, innerWitnessB witness.Witness, innerProofB groth16.Proof ) (groth16.Proof, groth16.VerifyingKey, witness.Witness) {
	...
	outerAssignment := &OuterCircuit_twoInner_same[sw_bls12377.ScalarField, sw_bls12377.G1Affine, sw_bls12377.G2Affine, sw_bls12377.GT]{
	...
	outerCircuit := &OuterCircuit_twoInner_same[sw_bls12377.ScalarField, sw_bls12377.G1Affine, sw_bls12377.G2Affine, sw_bls12377.GT]{
